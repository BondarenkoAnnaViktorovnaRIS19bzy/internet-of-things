...
class Identifier:
	def __init__(self):
		...
		self.friendly_names = {}
		self.allowed = []
		print('Загрузка распознанных')
		#load known people
		p = Path('people')
		if not (p.exists() and p.is_dir()):
			print(p,'Папка не существует')
			exit()

		for fn in p.glob('*.jpg'):
			name = fn.stem #название файла без расширения
		...
		if not(p.exists() and p.is_file()):
			print(p,'Файл не найден, или пуст')
			exit()
	...
	def getIDFromEncoding(self, encoding, difference=0.6):
		...
		print(uid, ' точность {:.1%}'.format(1-distances[most_similar]))
